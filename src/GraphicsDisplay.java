...
// Отобразить график
    ...
    protected boolean hasSumOfDigitsLessThanTen(double value) {
        ...
        int sum = 0;
        while (integerPart > 0) {
            sum += integerPart % 10; // Добавляем последнюю цифру
            integerPart /= 10;       // Убираем последнюю цифру
        }

        // Проверяем, меньше ли сумма 10
        return sum < 10;
    }
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : graphicsData) {
            ...
            // Выбор цвета в зависимости от суммы цифр
            if (hasSumOfDigitsLessThanTen(point[1])) {
            ...
            // Координаты вершин перевёрнутого треугольника
            int[] xPoints = new int[3];
            int[] yPoints = new int[3];

            // Верхняя вершина
            xPoints[0] = (int)(center.x - size / 2);
            yPoints[0] = (int)(center.y - size);

            // Нижняя левая вершина
            xPoints[1] = (int)(center.x + size / 2);
            yPoints[1] = (int)(center.y - size);

            // Нижняя вершина
            xPoints[2] = (int)(center.x);
            yPoints[2] = (int)(center.y + size);

            // Рисуем треугольник
            canvas.draw(new Polygon(xPoints, yPoints, 3));
        }
    }
